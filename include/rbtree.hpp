...
RBNode<T>* RBNode<T>::insert(std::unique_ptr<RBNode<T>>& n, const T& t) {
    ...
    else { //doesn't matter - already there
        n->key = t;
    }
...
RBNode<T>* RBNode<T>::fix_up(std::unique_ptr<RBNode<T>>& n) {
    ...
    if(is_red(n->right) && !is_red(n->left)){ //is_right(n->right) checks if n->right is nullptr since it returns n&&right==RED
...
RBNode<T>* RBNode<T>::remove(std::unique_ptr<RBNode<T>>& n, const T& t) {
    ...
    if(t<n->key){
        ...
        if(t == n->key && (!n->right)){ //delete
            return nullptr;
        }
        if(!is_red(n->right) && !is_red(n->right->left)){
            n.reset(move_red_right(n));
        }
        if(t == n->key){ //n->right != nullptr - right child must exist
